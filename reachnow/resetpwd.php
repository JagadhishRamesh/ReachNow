...
<ul >
  <li><a href="profile.php"><span class="glyphicon glyphicon-chevron-left"></span> Back</a></li>
...
<?php
   include('session.php');
    include ('navbar.html');
   $ses_sql = mysqli_query($db,"select userpwd from users where userid = '$login_userid' ");
   ...
   if(isset($_POST['submit']))
   {
   		if(md5($_POST['oldpwd'])== $old_pwd)
   		{
   			if($_POST['newpwd']== $_POST['confirmpwd'])
   			{
   				...
   				$updatepwd_sql="UPDATE users SET userpwd = '$updatepwd' WHERE userid = '$login_userid' ";
